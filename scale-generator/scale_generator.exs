defmodule ScaleGenerator do
  ...
  def step(scale, tonic, step) do
    ...
    |> (&(Enum.at(scale ++ scale, &1 + distance))).()
  end
  ...
  def chromatic_scale(tonic \\ "C") do
    do_generate_scale(@sharp_scale, tonic)
  end
  ...
  def flat_chromatic_scale(tonic \\ "C") do
    do_generate_scale(@flat_scale, tonic)
  end
  ...
  def scale(tonic, pattern) do
    ...
    pattern
    |> String.graphemes
    |> Enum.reduce([hd(scale)], fn(s, acc) ->
        tonic = Enum.fetch!(acc, -1)
        acc ++ [step(scale, tonic, s)]
       end)
  end

  defp do_generate_scale(base_scale, tonic) do
    {rest, start} = Enum.split_while(base_scale, &(&1 != String.capitalize(tonic)))
    start ++ rest
    |> Stream.cycle
    |> Enum.take(13)
  end
